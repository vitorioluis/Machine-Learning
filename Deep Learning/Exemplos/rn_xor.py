...
def model(X, Y, n_x, n_h, n_y, num_of_epocs, learning_rate, m):
    ...
    for i in range(0, num_of_epocs + 1):
        ...
        if(i % 100 == 0):
            print('Custa após interação # {:d}: {:f}'.format(i, cost))
...
def predict(X, parameters):
    ...
    y_predict = 1 if yhat >= 0.5 else 0
...
if __name__ == "__main__":
    ...
    n_x = 2      # Numeros de neurônios na primeira camada
    n_h = 3      # Numeros de neurônios na camada oculta
    n_y = 1      # Numeros de neurônios na camada de saída
    num_of_epocs = 1000  # Número de interações (epocas)
    learning_rate = 0.3  # taxa de aprendizagem


    trained_parameters = model(X, Y, n_x, n_h, n_y, num_of_epocs, learning_rate, m)
